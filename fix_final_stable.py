...
# 2. Implementar la versión definitiva de generate_dashboard_html
new_function = r'''
    def generate_dashboard_html(self, res_o, res_v, res_c, res_fb, df_metas):
        ...
        def prepare_json(df_in):
            ...
            def clean_col(c):
                # Solo caracteres alfanumericos y guion bajo para evitar errores en JS
                s = str(c).lower().strip()
                s = s.replace(" ", "_").replace("á", "a").replace("é", "e").replace("í", "i").replace("ó", "o").replace("ú", "u").replace("ñ", "n")
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            <div class="card h-[400px] flex flex-col">
                <h3 class="text-sm font-black mb-4 text-slate-400 uppercase tracking-widest border-b pb-2">Flujo de Leads</h3>
                <div id="ch-leads" class="flex-grow"></div>
            </div>
            <div class="card h-[400px] flex flex-col">
                <h3 class="text-sm font-black mb-4 text-slate-400 uppercase tracking-widest border-b pb-2">Ventas vs Meta</h3>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            <div class="card h-[400px] flex flex-col">
                <h3 class="text-sm font-black mb-4 text-slate-400 uppercase tracking-widest border-b pb-2">Ventas por Marca</h3>
                <div id="ch-marca" class="flex-grow"></div>
            </div>
            <div class="card h-[400px] flex flex-col">
                <h3 class="text-sm font-black mb-4 text-slate-400 uppercase tracking-widest border-b pb-2">Ventas por Asesor</h3>
                <div id="ch-asesor" class="flex-grow"></div>
            </div>
        </div>
    </div>

    <script id="data-block" type="application/json">DATA_PLACEHOLDER</script>
    <script>
        (function() {
            let rawData = null;
            try {
                const dataText = document.getElementById('data-block').textContent;
                rawData = JSON.parse(dataText);
                console.log("Dashboard: Datos cargados correctamente", rawData);
            } catch (e) {
                console.error("Dashboard: Error leyendo JSON", e);
                document.body.innerHTML = "<div class='p-20 text-red-500 font-black text-center'>ERROR: NO SE PUDIERON LEER LOS DATOS</div>";
                return;
            }
            ...
            function update() {
                ...
                document.getElementById("kpi-meta").innerText = tMet > 0 ? Math.round((tVta / tMet) * 100) + "%" : "0%";
                ...
                document.getElementById("kpi-vday").innerText = "Q " + Math.round(tVta / days).toLocaleString();
        ...
        html_final = html_template.replace("DATA_PLACEHOLDER", json.dumps(data_json))
        html_final = html_final.replace("TIMESTAMP_HERE", datetime.now().strftime("%d/%m/%Y %H:%M"))

        # Limpiar cualquier residuo de caracteres de escape que f-string podria haber dejado (aunque aqui no usamos f-string para el template)
...
if start != -1 and end != -1:
    ...
    print("Dashboard system completely rewritten for JSON reliability.")
else:
